...
/**
 * @typedef {import('react').CSSProperties & { [key: `--${string}`]: string | number }} FolderStyle
...
const Folder = ({ color = '#5227FF', size = 1, items = [], className = '', openDirection = 'up', onOpen }) => {
  ...
  const folderClassName = `folder ${openDirection === 'right' ? 'folder--right' : ''} ${open ? 'open' : ''}`.trim();
      ...
      >
        <div className="folder__back">
          {papers.map((item, i) => (
            <div
              key={i}
              className={`paper paper-${i + 1} paper-r${papers.length - i}`}
